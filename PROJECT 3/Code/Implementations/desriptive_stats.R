############################################################
# Descriptive EDA – final optimized version (10 plots)
# Discretization-aware, publication-grade
############################################################
...
# ----------------------------
# Paths (relative to Implementations/)
# ----------------------------
PLOT_DIR  <- "../Plots"
DATA_PATH <- "../processed_player_value/nn_tabular_dataset.csv"
...
p1 <- nn %>%
  ...
  geom_density(color = COL_RED, linewidth = 1.2, adjust = 1.1) +
  labs(
...
p3 <- nn %>%
  ...
  geom_density(fill = COL_RED, alpha = 0.6, adjust = 1.1) +
  labs(
...
############################################################
# 8. Goals vs assists (per match)
############################################################

p8 <- nn %>%
  select(mean_goals, mean_assists) %>%
  pivot_longer(everything(), names_to = "metric") %>%
  filter(value > 0) %>%
  mutate(value = value + runif(n(), 0, 1e-3)) %>%
  ggplot(aes(log1p(value), color = metric, fill = metric)) +
  geom_density(alpha = 0.25, linewidth = 1.2, adjust = 1.6) +
  scale_color_manual(values = c(
    mean_goals = COL_RED,
    mean_assists = COL_BLUE
  )) +
  scale_fill_manual(values = c(
    mean_goals = COL_RED,
    mean_assists = COL_BLUE
  ...
  labs(
    title = "Scoring contribution intensity",
    subtitle = "log(1 + events per match)",
    x = "log(1 + events per match)",
    y = "Density"
  )

save_pdf(p8, "08_goals_assists_density")

############################################################
# 9. Yellow vs red cards
############################################################

p9 <- nn %>%
  select(mean_yellow_cards, mean_red_cards) %>%
  pivot_longer(everything(), names_to = "card") %>%
  filter(value > 0) %>%
  mutate(value = value + runif(n(), 0, 1e-3)) %>%
  ggplot(aes(log1p(value), color = card, fill = card)) +
  geom_density(alpha = 0.30, linewidth = 1.2, adjust = 1.8) +
  scale_color_manual(values = c(
    mean_yellow_cards = COL_YEL,
    mean_red_cards = COL_RED
  )) +
  scale_fill_manual(values = c(
    mean_yellow_cards = COL_YEL,
    mean_red_cards = COL_RED
  ...
  labs(
    title = "Disciplinary intensity",
    subtitle = "Yellow vs red cards (per match)",
    x = "log(1 + cards per match)",
    y = "Density"
  )

save_pdf(p9, "09_cards_density")
...
p10 <- nn %>%
  select(sum_sub_in, sum_sub_out) %>%
  pivot_longer(everything(), names_to = "type") %>%
  filter(value > 0) %>%
  mutate(value = value + runif(n(), 0, 1e-3)) %>%
  ggplot(aes(log1p(value), color = type, fill = type)) +
  geom_density(alpha = 0.25, linewidth = 1.2, adjust = 1.6) +
  scale_color_manual(values = c(
    sum_sub_in = COL_BLUE,
    sum_sub_out = COL_RED
  ...
  labs(
    ...
    subtitle = "log(1 + cumulative events)",
    x = "log(1 + total substitutions)",
    y = "Density"
  )

save_pdf(p10, "10_substitutions_density")
